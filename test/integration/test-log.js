   ...
   return repo
      ...
      .then(() =>
         repo
            ...
            .then(data => (context.$userName = data.trim()))
      )
      .then(() =>
         repo
            .raw(["config", "user.email"])
            .then(data => (context.$userEmail = data.trim()))
      );
};

module.exports = {
   "multi-line commit message in log summary": new Test(
      setUp,
      (context, assert) => {
         return context
            .gitP(context.root)
   ...
   "multi-line commit message in custom format log summary": new Test(
      setUp,
      (context, assert) => {
         return context
            .gitP(context.root)
   ...
   "should read one line for each commit when using shortstat, with summary": new Test(
      setUp,
      (context, assert) => {
         return context
            .gitP(context.root)
            ...
            .then(actual => {
               ...
               assert.deepEqual(actual.all[0].diff, {
                  files: [],
                  changes: 1,
                  deletions: 0,
                  insertions: 1
               ...
               assert.deepEqual(actual.all[1].diff, {
                  files: [],
                  changes: 1,
                  deletions: 0,
                  insertions: 1
