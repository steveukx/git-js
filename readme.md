...
- `.addRemote(name, repo)` adds a new named remote to be tracked as `name` at the path `repo`
...
- `.listRemote([args])`  lists remote repositories - there are so many optional arguments in the underlying  `git ls-remote` call, just supply any you want to use as the optional `args` array of strings eg: `git.listRemote(['--heads', '--tags'], console.log.bind(console))`
