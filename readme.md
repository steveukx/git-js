...
| `.setChildProcess(ChildProcess)` | Overrides the current Chid Process with a new one. |
...
#Overriding Spawn command
```js
const git = require('simple-git/promise');

var ChildProcess = require("child_process");
ChildProcess.spawn = (function() {
    var cached_spawn = ChildProcess.spawn; //Make a cache of the existing spawn command.
    return function() {
        var args = ['sudo',[arguments['0']]]; //Adding sudo to all spawn commands.
        var keys = Object.keys(arguments);
        if(keys.length >= 2 && arguments['1'].length != null)
        {
            args[1] = args[1].concat(arguments['1'])
        }
        if(keys.length >= 3)
        {
            args.push(arguments['2']);
        }
        var result = cached_spawn.apply(this, args); //Use apply to call.
        return result;
    };
})();

git().setChildProcess(ChildProcess);
```
