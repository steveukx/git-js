...
| `.cherry([upstream, [head, [options, [handlerFn]]]])` | Find commits yet to be applied to upstream, meaning commits which are in head, but not in upstream. Upstream branch (can supply other refs too) is searched for equivalent commits, defaults to upstream of current HEAD. Head is working branch (or other ref), defaults to current HEAD. When supplied, the [options](#how-to-specify-options) argument is expected to be any options accepted by [git-cherry](https://git-scm.com/docs/git-cherry). |
