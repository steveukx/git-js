...
| `.checkout(checkoutWhat, handlerFn)` | checks out the supplied tag, revision or branch. `checkoutWhat` can be one (`String`) or more strings (`Array`) to be used as parameters appended to the `git checkout` command. |
