...
debug.formatters.L = (value: unknown) => String(filterHasLength(value) ? value.length : '-');
...
type OutputLoggingHandler = (message: string, ...args: unknown[]) => void;
...
function childLoggerName(
   ...
   const childNamespace = childDebugger?.namespace || '';

   if (childNamespace.startsWith(parentNamespace)) {
      return childNamespace.substring(parentNamespace.length + 1);
   }
...
export function createLogger(
   ...
   initialStep?: string,
   infoDebugger: Debugger = createLog()
): OutputLogger {
...
/**
 * The `GitLogger` is used by the main runner to handle logging any warnings
 * or errors.
 */
