...
import { asStringArray, getTrailingOptions } from '../utils';
...
export function resetTask(mode: Maybe<ResetMode>, customArgs: string[]): StringTask<string> {
...
export function reset(mode?: ResetMode | unknown, ...args: unknown[]): StringTask<string> {
   return resetTask(getResetMode(mode), getTrailingOptions([mode, ...args]));
}
