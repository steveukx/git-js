...
export function cleanWithOptionsTask(
   ...
   customArgs: string[]
): StringTask<CleanSummary> | EmptyTask {
...
export function clean(
   mode?: CleanMode | CleanOptions[] | unknown,
   ...args: unknown[]
): StringTask<CleanSummary> | EmptyTask {
   const usingCleanOptionsArray = isCleanOptionsArray(mode as CleanOptions[]);
   const cleanMode =
      (usingCleanOptionsArray && (mode as CleanOptions[]).join('')) ||
      filterType(mode, filterString) ||
      '';
   const customArgs = getTrailingOptions(usingCleanOptionsArray ? args : [mode, ...args]);

   return cleanWithOptionsTask(cleanMode, customArgs);
}
