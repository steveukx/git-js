...
describe('plugin.unsafe', () => {
   ...
   describe('Command injection through .env', () => {
      ...
      it('blocks core.fsmonitor by default', async () => {
         ...
         const unsafeResult = await promiseResult(
            // the unsafe flag clears the argv-parser vulnerability check; the
            // deny-by-default config-write guard (§2.7) still requires the key
            // to be allow-listed to perform the write
            newSimpleGit(context.root, {
               unsafe: { allowUnsafeFsMonitor: true },
               allowConfigWrite: ['core.fsmonitor'],
            }).raw('-c', `core.fsmonitor=touch ${pwnedPath()}`, 'status')
         );
   ...
   it('allows overriding protocol when opting in to unsafe practices', async () => {
      const { threw } = await promiseResult(
         // unsafe flag + config-write allow-list are both required (§2.7)
         newSimpleGit(context.root, {
            unsafe: { allowUnsafeProtocolOverride: true },
            allowConfigWrite: ['protocol.ext.allow'],
         }).raw('-c', 'protocol.ext.allow=always', 'init')
      );
