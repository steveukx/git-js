/**
 * Ported from v3 `test/unit/plugins/plugin.abort.spec.ts`. The 'aborts all
 * steps in chained promises' test used v3's method-chaining (`a.raw('b')`) -
 * until the chainable api lands it exercises the same serial-chain semantics
 * through `git.run(...)`.
 */
import { promiseError } from '@kwsites/promise-result';

import { GitPluginError } from '../../../src/errors';
import { straightThroughStringTask } from '../../../src/tasks';
import {
...
describe('plugin.abort', () => {
   ...
   it('aborts all steps in a serial chain of tasks', async () => {
      const { controller, abort } = createAbortController();
      const git = newSimpleGit({ abort });
      ...
      const queue = git
         .run(
            ...
            straightThroughStringTask(['b']),
            straightThroughStringTask(['c'])
         )
         .catch((e) => e);
      ...
      expect(await queue).toEqual(expect.any(GitPluginError));
      assertExecutedTasksCount(1);
   });
