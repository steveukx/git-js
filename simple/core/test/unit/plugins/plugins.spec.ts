/**
 * Ported from v3 `test/unit/plugins/plugins.spec.ts`. Sugar-method calls
 * (`git.init()`, `git.push()`, `git.checkout(...)`) are supplied through
 * `git.raw` until the binding table lands - the progress plugin keys off the
 * task's first command, so the auto-add assertions are equivalent.
 */
import type { Mock } from 'vitest';

import type { SimpleGitCore } from '../../../src/git';
import {
...
describe('plugins', () => {
   ...
   describe('spawnOptions', () => {
      ...
      it('sets process ids along with environment variables', async () => {
         ...
         assertChildProcessSpawnOptions({
            gid: 2,
            env: expect.objectContaining({ hello: 'world' }),
         });
      });
   });
