/**
 * Ported from v3 `test/unit/plugins/plugin.unsafe.spec.ts`. Cases exercising
 * the `clone` / `pull` / `push` / `listRemote` task factories (including the
 * clone pathspec-protection cases) follow with the task-porting phase - the
 * argv-level assertions here are unchanged, supplied through `git.raw`.
 *
 * Per the deny-by-default config-write guard, the "allows with override"
...
describe('blockUnsafeOperationsPlugin', () => {
   it.each([
      ...
      const err = promiseError(
         newSimpleGit({
            ...
            allowConfigWrite: ['protocol.allow'],
         }).raw(...task)
      );
   ...
   describe.each([
      ...
      it('blocks by default', async () => {
         const err = promiseError(newSimpleGit().raw('clone', '-c', command, 'remote', 'local'));
         await promiseError(closeWithSuccess());
      ...
      it('allows with override', async () => {
         const err = promiseError(
            newSimpleGit({
               unsafe: { [setting]: true },
               allowConfigWrite: [command.split('=')[0]],
            }).raw('clone', '-c', command, 'remote', 'local')
         );
         await closeWithSuccess();
   ...
   it('uses pathspec protection for -u in remote', async () => {
      const git = newSimpleGit({ unsafe: {} });
      const err = promiseError(
         git.raw('clone', pathspec('-u touch /tmp/pwn', 'file:///tmp/zero12'))
      );
      await promiseError(closeWithSuccess());
