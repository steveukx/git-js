...
import type { MergeResult, SimpleGitCore } from '../../index';
import { parseMergeResult } from '../../src/parsers/parse-merge';
import { MergeSummaryDetail } from '../../src/responses/MergeSummary';
import {
...
describe('merge', () => {
   describe('api', () => {
      ...
      it('merge with fatal error', async () => {
         ...
         git.mergeFromTo('aaa', 'bbb', 'x' as any);
...
`);
      ...
      it('responds with a MergeResult', async () => {
         const queue = git.mergeFromTo('alpha', 'beta');
